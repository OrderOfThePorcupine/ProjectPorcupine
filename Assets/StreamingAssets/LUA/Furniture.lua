...
function Stockpile_JobWorked(job)
	...
	for k, inv in pairs(job.DeliveredItems) do
		if(inv.StackSize > 0) then
			GameController.CurrentWorld.InventoryManager.PlaceInventory(job.tile, inv)
...
function MiningDroneStation_JobComplete(job)
	if (job.buildable.Jobs.OutputSpotTile.Inventory == nil or job.buildable.Jobs.OutputSpotTile.Inventory.Type == job.buildable.Parameters["mine_type"].ToString()) then
		GameController.CurrentWorld.InventoryManager.PlaceInventory(job.buildable.Jobs.OutputSpotTile, Inventory.__new(job.buildable.Parameters["mine_type"].ToString(), 2))
	else
		job.CancelJob()
	end
end
	...
	if (inputSpot.Inventory ~= nil and inputSpot.Inventory.StackSize >= 5) then
		...
		if (furniture.Parameters["smelttime"].ToFloat() >= furniture.Parameters["smelttime_required"].ToFloat()) then
			...
			if (outputSpot.Inventory == nil) then
				GameController.CurrentWorld.InventoryManager.PlaceInventory(outputSpot, Inventory.__new("Steel Plate", 5))
...
function MetalSmelter_JobWorked(job)
	...
	for k, inv in pairs(job.DeliveredItems) do
		if(inv ~= nil and inv.StackSize > 0) then
			GameController.CurrentWorld.InventoryManager.PlaceInventory(inputSpot, inv)
...
function CloningPod_JobComplete(job)
	GameController.CurrentWorld.CharacterManager.Create(job.buildable.Jobs.OutputSpotTile)
...
function Heater_InstallAction( furniture, deltaTime)
	...
	GameController.CurrentWorld.temperature.RegisterSinkOrSource(furniture)
end

-- This function gets called once, when the furniture is uninstalled
function Heater_UninstallAction( furniture, deltaTime)
	GameController.CurrentWorld.temperature.DeregisterSinkOrSource(furniture)
...
function AirPump_OnUpdate(furniture, deltaTime)
	...
	if (furniture.Parameters["active"].ToBool()) then
		...
		local north = GameController.CurrentWorld.GetTileAt(t.X, t.Y + 1, t.Z)
		local south = GameController.CurrentWorld.GetTileAt(t.X, t.Y - 1, t.Z)
		local west = GameController.CurrentWorld.GetTileAt(t.X - 1, t.Y, t.Z)
		local east = GameController.CurrentWorld.GetTileAt(t.X + 1, t.Y, t.Z)
...
function Berth_TestSummoning(furniture, deltaTime)
	...
	elseif (GameController.CurrentWorld.ShipManager.IsOccupied(furniture)) then
...
function Berth_SummonShip(furniture, character)
	...
	local ship = GameController.CurrentWorld.ShipManager.AddShip("essentia", 0, 0)
	ship.SetDestination(furniture)
end

function Berth_DismissShip(furniture, character)
	local shipManager = GameController.CurrentWorld.ShipManager
...
function OreMine_OreMined(job)
	...
	if (inventory.Type ~= "None") then
		...
		GameController.CurrentWorld.InventoryManager.PlaceInventory(job.tile, inventory)
	end
...
function Berth_SummonShip(furniture, character)
	...
	if (ship.WouldFitInBerth(furniture)) then
		...
		GameController.CurrentWorld.ShipManager.RemoveShip(ship)
		furniture.Parameters["occupied"].SetValue(0)
	end
end
